...
class LibraryComponent extends React.Component {
  ...
  handleSelect(id) {
    this.handleClose();
    this.props.onItemSelected(this.getFilteredData().find((item) => this.constructKey(item) === id));
  }
  handleClose() {
  ...
  handleTagClick(tag) {
    if (this.state.playingItem === null) {
      ...
      this.props.onItemMouseLeave(
        this.getFilteredData().find((item) => this.constructKey(item) === this.state.playingItem)
      );
      this.setState({
        filterQuery: '',
        playingItem: null,
  ...
  handleMouseEnter(id) {
    ...
    if (this.props.onItemMouseEnter && this.state.playingItem !== id) {
      this.props.onItemMouseEnter(this.getFilteredData().find((item) => this.constructKey(item) === id));
      this.setState({
        playingItem: id
      });
    }
  ...
  handleMouseLeave(id) {
    if (this.props.onItemMouseLeave) {
      this.props.onItemMouseLeave(this.getFilteredData().find((item) => this.constructKey(item) === id));
      this.setState({
        playingItem: null
      });
    }
    ...
    return Object.entries(dataByCategory)
      ...
      .map(([key, values]) => (
        <div key={key} className={styles.libraryCategory}>
          {key === 'undefined' ? null : (
            <span className={styles.libraryCategoryTitle}>{this.props.intl.formatMessage(messages[key])}</span>
          )}
          <div className={styles.libraryCategoryItems}>{values.map((item) => this.renderElement(item))}</div>
        </div>
      ));
  }
  render() {
