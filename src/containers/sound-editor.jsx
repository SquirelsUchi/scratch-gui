  ...
  submitNewSamples(samples, sampleRate, skipUndo) {
    return downsampleIfNeeded({ samples, sampleRate }, this.resampleBufferToRate)
      .then(({ samples: newSamples, sampleRate: newSampleRate }) => {
        const mp3Buffer = encodeMp3(newSamples, newSampleRate);
        ...
        if (sound.soundId && vm.runtime && vm.runtime.audioEngine) {
          vm.editingTarget.sprite.soundBank.getSoundPlayer(sound.soundId).buffer = this.audioBufferPlayer.buffer;
        }

        sound.format = '';
        sound.asset = storage.createAsset(storage.AssetType.Sound, storage.DataFormat.MP3, mp3Buffer, null, true);
        sound.assetId = sound.asset.assetId;
        sound.dataFormat = storage.DataFormat.MP3;
        sound.md5 = `${sound.assetId}.${sound.dataFormat}`;
        sound.sampleCount = newSamples.length;
        sound.rate = newSampleRate;
