...
import { openSpriteLibrary, closeSpriteLibrary, openFileExcideModal, openCorruptFileModal } from '../reducers/modals';
...
import { validateFileContent } from '../shared/utils/file-validation/contentValidation.js';
import { compressImage } from '../shared/utils/files-compress/imagesCompress.js';

const SPRITE_FILE_ACCEPT = ['.png', '.jpg', '.jpeg', '.gif'];
...
class TargetPane extends React.Component {
  ...
  handleChangeSpriteSize(size) {
    ...
    this.props.vm.postSpriteInfo({ size: value });
  }
  ...
  handleChangeSpriteX(x) {
    ...
    if (Number.isNaN(value)) return;
    this.props.vm.postSpriteInfo({ x: value });
  }
  handleChangeSpriteY(y) {
    const value = Number(y);
    if (Number.isNaN(value)) return;
    this.props.vm.postSpriteInfo({ y: value });
  }
  ...
  async handleSpriteUpload(e) {
    ...
    const contentValidFiles = await validateFileContent(
      compressedTarget,
      SPRITE_FILE_ACCEPT,
      this.props.onOpenCorruptFileModal
    );
    if (contentValidFiles.length === 0) {
    ...
    handleFileUpload(
      ...
      (buffer, fileType, fileName, fileIndex, fileCount) => {
        spriteUpload({
          ...
          spriteName: fileName,
          storage,
          onFileExceedSize: this.props.onOpenFileExcideModal,
          ...
          handleError: this.props.onCloseImporting
        });
      },
      this.props.onCloseImporting
    );
  ...
  render() {
    ...
    const {
      ...
      onReceivedBlocks,
      onShowImporting,
      onOpenFileExcideModal,
      onOpenCorruptFileModal,
      workspaceMetrics,
      ...componentProps
    } = this.props;
...
TargetPane.propTypes = {
  ...
  onOpenFileExcideModal: PropTypes.func,
  onOpenCorruptFileModal: PropTypes.func,
  ...targetPaneProps
};
...
const mapDispatchToProps = (dispatch) => ({
  ...
  onOpenFileExcideModal: () => dispatch(openFileExcideModal()),
  onOpenCorruptFileModal: () => dispatch(openCorruptFileModal())
});
