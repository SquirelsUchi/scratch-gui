...
import { isSingleFileSizeValid } from '../shared/utils/file-validation/fileValidation.js';
import { openFileExcideModal } from '../reducers/modals.js';
  ...
  if (costumeFormat === storage.DataFormat.SVG) {
    ...
    bitmapAdapter
      ...
      .then(async (processedData) => {
        ...
        const blob = processedData instanceof Blob ? processedData : new Blob([processedData]);
        ...
        const isFileSizeValid = isSingleFileSizeValid(processedBytes);

        if (!isFileSizeValid) {
          onFileExceedSize(openFileExcideModal());
          handleError();
        } else {
          addCostumeFromBuffer(rawData);
        }
      })
      .catch(handleError);
  }
